...
- **I1.2 Normalization is reported, not performed.** L1 detects text that is
  ...
  `mdfix/vendor/utf_nfc.c`; taking that dependency found two defects in it,
  both since fixed upstream and carried by the current copy —
  brazilofmux/utf#1 (a run of composition exclusions truncated in silence)
  and #2 (truncation unreportable). mdfix's workaround for the first is
  gone.)*
...
- **I3.1 Non-interference.** For every optional transform, applied alone and
  ...
  a test matrix, not a promise — `tests/test_transform_matrix.py` sweeps every
  transform over every document and pins the violations exactly, so both a new
